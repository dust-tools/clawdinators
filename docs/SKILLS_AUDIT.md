...
Goal: keep the headless CLAWDINATOR surface minimal and relevant. Bundled skills include many desktop- or device-specific integrations that don’t make sense on a server.
...
Suggested allowlist for CLAWDINATOR:
- `github` (issues/PRs via gh)
- `clawdhub` (install/update skills on demand)
...
```
services.clawdinator.config.skills.allowBundled = [ "github" "clawdhub" ];
```
